  ...
  <main>
    ...
    <form @submit.prevent="submit()">
      ...
      <div class="form-fil" v-for="(x, index) in listImage" :key="index">
        <label :for="index" class="file" v-if="index > 0 && x == ''"
          >Chọn hình ảnh sản phẩm</label
        >
        <label :for="index" class="file" v-if="index == 0 && x == ''"
          >Chọn hình ảnh đại diện</label
        >
        <label :for="index" class="file" v-if="x != ''">{{ x }}</label>
        <input
          :id="index"
...
import { addProduct } from "../configs/firebase";
export default {
  setup() {
